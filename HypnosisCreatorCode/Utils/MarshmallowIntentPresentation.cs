...
/// <summary>
/// マシュマロ回答中の不安全敵は UI だけ防御意図を表示する（NextMove は変更しない）。
/// </summary>
public static class MarshmallowIntentPresentation
{
    public static bool ShouldOverride(Creature? creature) =>
        ...
        && creature.GetPower<MarshmallowDefendPower>() is { ShouldReplacePerform: true };

    public static Task UpdateIntent(NCreature node, IEnumerable<Creature> targets)
    {
        return ApplyIntents(node, targets, [new DefendIntent()]);
    }
