...
{
  Console.WriteLine("\n===== " + label + " =====");
  if (move == null) { Console.WriteLine("NULL"); return; }
  ...
  for (var i = 0; i < il.Length; i++)
  {
    ...
    if (op == 0x15) { Console.WriteLine("ldc.i4.m1"); continue; }
    ...
    if (op == 0x14) { Console.WriteLine("ldnull"); continue; }
    if (op == 0x72) { var t=BitConverter.ToInt32(il,i+1); try{Console.WriteLine($"ldstr \"{move.Module.ResolveString(t)}\"");}catch{} i+=4; continue; }
    if (op is 0x28 or 0x6F or 0x73)
    {
      ...
      try {
        ...
        Console.WriteLine($"call {m.DeclaringType?.Name}.{m.Name}{g}");
      } catch {}
      i+=4; continue;
    }
    ...
    {
      var t=BitConverter.ToInt32(il,i+1);
      try { var f=move.Module.ResolveField(t); Console.WriteLine($"{(op==0x7B?"ldfld":"stfld")} {f.DeclaringType?.Name}.{f.Name}"); } catch {}
      i+=4; continue;
    }
  }
}
...
foreach (var prop in new[]{"Allies","Enemies"})
{
  ...
  Console.WriteLine($"\n{prop} exists={g!=null} len={g?.GetMethodBody()?.GetILAsByteArray()?.Length}");
  Dump(g, "get_"+prop);
}
...
Console.WriteLine("\n===== types containing MACHINE =====");
foreach (var t in types.Where(t => t.Name.Contains("Machine", StringComparison.OrdinalIgnoreCase)).OrderBy(t=>t.FullName))
  Console.WriteLine(t.FullName);

Console.WriteLine("\n===== IsSecondaryEnemy power check =====");
Dump(T("Creature").GetProperty("IsSecondaryEnemy")!.GetMethod!, "IsSecondaryEnemy");
Dump(T("MinionPower").GetProperty("OwnerIsSecondaryEnemy")!.GetMethod!, "MinionPower.OwnerIsSecondaryEnemy");
