...
foreach (var m in cm.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly).Where(x => x.Name == "CheckWinCondition"))
{
    ...
    for (var i = 0; i < il.Length; i++)
    {
        if (il[i] is 0x28 or 0x6F or 0x73)
        {
            try
            {
                ...
                if (!n.Contains("AsyncTask") && !n.Contains("TaskAwaiter") && !n.Contains("SetResult") && !n.Contains("Await") && !n.Contains("Enumerator") && !n.Contains("MoveNext") && !n.Contains("Dispose") && !n.Contains("ExecutionContext"))
                    Console.WriteLine(n);
            }
            catch { }
            i += 4;
