...
class P {
  ...
  static void Main() {
    ...
    var hand = a.GetTypes().First(x => x.Name == "NPlayerHand");
    Dump(hand.GetMethod("AreCardActionsAllowed", BindingFlags.Public|BindingFlags.NonPublic|BindingFlags.Instance|BindingFlags.DeclaredOnly)!);
    var nc = a.GetTypes().First(x => x.Name == "NClickableControl");
    foreach (var e in nc.GetEvents()) Console.WriteLine("EVENT " + e.Name + " : " + e.EventHandlerType);
  }
}
