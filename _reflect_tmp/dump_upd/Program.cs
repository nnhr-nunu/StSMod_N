...
class P {
  static void Dump(MethodInfo m) {
    ...
    for (int i = 0; i < il.Length; i++) {
      byte op = il[i];
      if (op is 0x28 or 0x6F or 0x6A or 0x72) {
        int token = BitConverter.ToInt32(il, i+1);
        try { var member = module.ResolveMember(token); Console.WriteLine((op switch{0x28=>"call",0x6F=>"callvirt",0x6A=>"ldstr",0x72=>"ldstr",_=>"?"}) + " " + member); } catch {}
        i += 4;
      } else if (op == 0x2B) i += 1;
    }
  }
  static void Main() {
    ...
    Dump(inspect.GetMethod("UpdateRelicDisplay", BindingFlags.Public|BindingFlags.NonPublic|BindingFlags.Instance|BindingFlags.DeclaredOnly)!);
  }
}
