...
class P {
  ...
  static void Main() {
    ...
    var relic = a.GetTypes().First(x => x.Name == "NRelic");
    Dump(relic.GetMethod("_Ready", BindingFlags.Public|BindingFlags.NonPublic|BindingFlags.Instance|BindingFlags.DeclaredOnly)!);
    var holder = a.GetTypes().First(x => x.Name == "NRelicInventoryHolder");
    Dump(holder.GetMethod("_Ready", BindingFlags.Public|BindingFlags.NonPublic|BindingFlags.Instance|BindingFlags.DeclaredOnly)!);
  }
}
